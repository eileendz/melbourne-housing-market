---
title: "Melbourne housing market: Price modelling ETC3555 report"
...
[Sherry's part that is relevant to the nn architecture]
...
Kauko[reference] has provided an overview on current neural network application for predicting house prices and introduced some advanced methods including self-organising map (SOM) and learning vector quantization (LVQ). However there is no pre-trained architecture for house price forecasting and thus we build our own two layer deep neural network from scratch. Based on the size of our data (48k observations with 277 features) and the project goal (classifying house price into 10 categories), it is not sensible to construct a neural network with more than 4 layers. Neural network with 3 layers may also work, but adding an additional layer increases the computational requirements for the tuning of a model that was already too heavy for our computers to handle. This is a possible extension of our study.

```{r fig-architecture, fig.cap="Neural network architecture."}
...
In running our model normalisation is performed before each dense layer. This normalisation is necessary because it helps to speed up the converging process by normalising the extreme large value outputted from the previous layer. 
...
2) unlike tanh function which will saturate at the asymptotic, which means the weight will not be updated much when the input neural has a large value, Relu functions will increase to infinity. This property of relu allows the relevant weight to be updated when the input gets larger. And the potential drawback of getting extremely large output is overcome through the aforementioned batch normalisation.

The activation function for the final layer is softmax as this is the appropriate function for multi-level classification as it bounds probability ranges between 0-1, and the sum of probability for inclusion into each category will sum to 1. 

To avoid overfitting of the data, several regularisation methods are used. Dropout rate is used before each dense layer. One thing to notice is that the dropout rate is added before the batch normalisation because otherwise the normalisation will be based on the full data while part of them will be dropped out before supplied to the next layer. Regularisation is also applied in each layer with learning rate being a hyper-parameter to tune. 

Early stopping is also used to allow for more efficient updating on the parameter. We set max epochs at a large fixed value of 500 and use `patience = 20` for the early stopping in our network structure. This design allows the neural network to continue updating if needed while, not wasting computational power and time when the updates become slow.
...
<!-- how to select benchmark model -->


```{r price-class, fig.height=2}
...
```{r best, fig.cap="Screenshot of the training with the lowest validation loss."}
include_graphics("figure/best.png")
```

```{r best-para}
kable(tibble(hyperparameter = c("dense_unit1", "dense_unit2", "dropout", "batch_size", "activation"),
       value = c(256, 64, 0.2, 256, "relu")),caption = "\\label{tab:best-para}")
```

This model is then performed on the test set and the prediction matrix is presented in Table \ref{tab:pred-matrix-nn}. The accuracy of neural network algorithm is 50%. The multinomial logistic model is also performedon the test set and the accuracy is 51%. Associated prediction matrix is presented in Table \ref{tab:pred-matrix-mnl}. 
